...
int main(int argc, char **argv)
{
    ...
    long long int *window_buffer;
    if (world_rank == 0)
    {
        // Main
        // Allocate memory for the target window
        MPI_Alloc_mem(sizeof(long long int), MPI_INFO_NULL, &window_buffer);

        *window_buffer = number_in_circle;
        
        // int MPI_Win_create(void *base, MPI_Aint size, int disp_unit, MPI_Info info,
        //                MPI_Comm comm, MPI_Win *win)
        MPI_Win_create(window_buffer, sizeof(long long int), 
                    sizeof(long long int), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
    }
    else
    {
        // Workers
    ...
    if (world_rank == 0)
    {
        ...
        pi_result = 4.0 * (*window_buffer) / tosses;
