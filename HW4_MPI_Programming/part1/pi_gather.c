...
int main(int argc, char **argv)
{
    ...
    // TODO: use MPI_Gather
    // 用 MPI_Gather 將所有行程的 number_in_circle 收集到 rank 0 的 all_counts 中
    // int MPI_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
    //            void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
    MPI_Gather(&number_in_circle, 1, MPI_LONG_LONG_INT,
               all_counts, 1, MPI_LONG_LONG_INT, 0, MPI_COMM_WORLD);
    ...
    if (world_rank == 0)
    {
        ...
        {
            total_number_in_circle += all_counts[i];
        }
